...
fetch_cmd = on_command("fetch_members", aliases={"爬取群成员"}, permission=SUPERUSER, priority=10, block=True)

@fetch_cmd.handle()
async def handle_fetch(bot: Bot, event: MessageEvent, args: Message = CommandArg()):
    ...
    if not member_list:
        await fetch_cmd.finish("未获取到任何成员信息")
        return
